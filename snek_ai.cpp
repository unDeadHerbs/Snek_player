...
#define DBG2(X, SYM)                                                           \
  do {                                                                         \
    if (udh::cio[(X).first][(X).second] == ' ' or                              \
        udh::cio[(X).first][(X).second] == '.' or                              \
        udh::cio[(X).first][(X).second] == ',' or                              \
...
#define CLEAR()                                                                \
  do {                                                                         \
    while (dots.size()) {                                                      \
      auto d = dots.front();                                                   \
      dots.pop();                                                              \
      if (udh::cio[d.first][d.second] == '.' or                                \
          udh::cio[d.first][d.second] == ',' or                                \
          udh::cio[d.first][d.second] == '_')                                  \
...
Point &operator+=(Point &pnt, Direction const dir) { return pnt = pnt + dir; }

Direction operator-(Point const to, Point const from) {
  for (auto const dir :
...
enum Rotate { left, straight, right, backwards };
Direction operator*(Direction const dir, Rotate const rot) {
  switch (dir) {
  case Direction::none:
    return Direction::none;
  case Direction::up:
    ...
    case Rotate::straight:
      return Direction::up;
    case Rotate::right:
      return Direction::right;
    case Rotate::backwards:
...
Rotate operator/(Direction const to, Direction const from) {
  for (auto rot : {left, straight, right, backwards})
    if (to == from * rot)
      return rot;
  return straight; // TODO: throw?
}

int unreachable_count(std::deque<Point> walls, Point from, Point border) {
  CLEAR3();
  std::queue<Point> to_consider{{from}};
  while (!to_consider.empty()) {
    // DBG("--- progress
    // "<<to_consider.size()<<"/"<<walls.size()<<"/"<<border.first*border.second<<"\n");
    ...
    for (auto dir :
      ...
      if (std::find(walls.begin(), walls.end(), test) != walls.end())
        continue;
      if (test.first <= 0 or test.second <= 0 or test.first > border.first or
          test.second > border.second)
        continue;
...
bool reachable_flood(std::deque<Point> walls, Point from, Point to,
                     Point border) {
  CLEAR3();
  if (from == to)
    return true;
...
bool reachable(std::deque<Point> const walls, Point from, Point to,
               Point boarder) {
...
int snek_aware_distance(Snek const &game, Point goal) {
  ...
  while (pnt != goal) {
    DBG3(pnt);
    if (auto intersect = std::find(walls.begin() + 1, walls.end(), pnt);
        intersect != walls.end()) {
      CLEAR3();
    ...
    if (xdist > ydist)
      if (pnt.first > goal.first)
        ...
        pnt += Direction::right;
    else if (pnt.second > goal.second)
      pnt += Direction::up;
    else
      pnt += Direction::down;
  }
...
int body_to_goal_dist(std::deque<Point> body, Point goal) {
...
int wall_count(std::deque<Point> const &walls, Point const pnt,
               Point const border) {
  int c = 0;
  for (auto dir :
       {Direction::up, Direction::right, Direction::down, Direction::left}) {
    Point t = pnt + dir;
    if (t.first <= 0 or t.second <= 0 or t.first > border.first or
        t.second > border.second) {
      c++;
      continue;
    }
...
public:
  ...
  void push(T elm) {
    ...
    while (std::next(ptr) != list.end() and
           ((GREATER and std::next(ptr)->second > v) or
            (!GREATER and std::next(ptr)->second < v)))
      ++ptr;
...
auto mk_AI(
    std::function<
        std::function<int(std::pair<std::vector<Action>, State>)>(State)>
        metric,
    std::function<
        std::function<bool(std::pair<std::vector<Action>, State>)>(State)>
        should_cut,
    std::function<
        std::function<bool(std::pair<std::vector<Action>, State>)>(State)>
        is_done,
    std::function<std::vector<std::pair<std::vector<Action>, State>>(
  ...
  auto AI = [=](State const &state) -> std::vector<Action> {
    ...
    priority_stack<Consideration, std::function<int(Consideration)>, false,
                   SS == BFS>
 ...
 * be ignored/rebuilt more frequently, making them more ephemeral and not
 * needing of avoidance.  It may be the case that the interlocking structures
 * decrease the average time to the next apple.)
 *
 * This solver rarely finishes a game because once it makes a pocket
 * of unit size it has a very hard time filling it.  Particularly in
 * the end game, it will have a few unit pockets left and no way to
 * move them together and close them.
 */
std::function<std::vector<Direction>(Snek const &)> Snek_AI_go_right() {
  ...
  auto metric = [](Snek initial) {
    ...
    return [=](Consideration con) -> int {
      ...
      if (goal_dist == goal_dist_smart)
        return goal_dist + depth;
      // Else take the path as enumerated.
      return int(game.Size().first * game.Size().second) +
             depth / int(2 * b.size());
      // If the path is longer than twice the body, check for alternatives.
    };
  };
  auto cutter = [](Snek) {
    return [](Consideration con) -> bool {
      ...
      if (!game.Alive())
        return true;
      // TODO: Cut if a pocket of size 1 would be
      // created.  This sort of belongs in a
      // different AI name, but should have this
      // AI actually complete the game reliably,
      // and so I'm fine with adding it to help
      // the metrics.
      if (!reachable(body, body[0], body.back(), game.Size()))
        if (body.size() < game.Size().first * game.Size().second - 1)
          return true;
      if (path.size() > game.Size().first * game.Size().second * 4)
        return true; // cutting deep trees just so that it gives up
                     // when it's stuck.
      // TODO: Cut instead if a pattern of
      // body.size() +5 or -0 repeats.
      return false;
    };
  };
  ...
  auto enumerator = [](Consideration con) -> std::vector<Consideration> {
    ...
    if (con.first.size() == 0) {
      for (auto dir : {Direction::up, Direction::right, Direction::down,
                       Direction::left}) {
        Path p(con.first);
        Snek g(con.second);
        p.push_back(dir);
      ...
      for (auto dir : {con.first.back() * left, con.first.back(),
                       con.first.back() * right}) {
        Path p(con.first);
        Snek g(con.second);
        p.push_back(dir);
        g.move(dir);
...
std::function<std::vector<Direction>(Snek const &)> Snek_AI_no_pockets() {
  ...
  auto metric = [](Snek initial) {
    ...
    return [=](Consideration con) -> int {
      ...
      auto goal_dist_smart = snek_aware_distance(game, goal);
      auto depth = int(path.size());
      auto unreachable = unreachable_count(b, b[0], game.Size());
      if (depth < 3)
        // escape the head
        return -1;
      if (goal_dist == goal_dist_smart)
        return goal_dist + depth + unreachable;
      // A* with a small weighting
      return goal_dist + 10 * (goal_dist + depth) + wiggles + turns +
             unreachable;
    };
  };
  auto cutter = [](Snek initial) {
    ...
    return [=](Consideration con) -> bool {
      ...
      if (!reachable(body, body[0], body.back(), game.Size()))
        return true;
      if (body[0] == goal)
        if (wall_count(body, goal, game.Size()) > 2 and
            distance(goal, body.back(), 1) < 3)
  ...
  auto metric=[](Point goal){
                return [=](Consideration con)->int{
                         ...
                         if(depth<3)
                           ...
                           return -200000+depth;
                         if(is_simple_path)
                           // take simple path with small preference to
developed paths return -100000+10.1*goal_dist+10*depth;
                         ...
                         int distance_guess           =   40 *
(goal_dist+depth); int less_distance_if_wiggles =  -30 *
goal_dist*(turns>10)*(turns<b.size()/4); int follow_walls             =   30 *
wall_count(b,b[0],con.game.Size());
                         // TODO: Make follow_walls a property of the path not
the head.
                         // int avoid_small_bubbles   =   20 *
bubble_count(con.game); int unprefer_undeveloped     =   15 * goal_dist; int
prefer_developed_paths   =  -10 * depth; int prefer_lots_of_turns     =  -15 *
turns*(turns>5); int prefer_less_turns        =   10 * (turns/(depth/20+1)
                                                                +b[0].first!=goal.first+b[0].second!=goal.second);
                         int when_lost_find_tail      =   -1 *
(tail_lin_dist+tail_dist); int developed_simple_path    =    1 * goal_dist *
(goal_dist==goal_lin_dist);

                         return
                           distance_guess+prefer_less_turns+when_lost_find_tail
                           +prefer_developed_paths
+follow_walls+prefer_lots_of_turns
                           +less_distance_if_wiggles+follow_tail_if_near+unprefer_undeveloped
                           +developed_simple_path;
                       };
              };
  priority_stack<Consideration,decltype(metric(food)),false>
possibilities(metric(food)); possibilities.push({{},game});
  while(!possibilities.empty()){
    auto trying=possibilities.pop();
    if(possibilities.empty())
      if(trying.path.size()){ // since all must descend from this, lets advance
the board DBG("-- Short Cutting\n"); return trying.path;
      }
    DBG("- considering point\n");
    DBG2(trying.game.Body()[0],'_');
    for (auto dir :
           {Direction::up, Direction::right, Direction::down, Direction::left})
{ DBG("-- consider point "<<dir<<"\n"); Snek t_game(trying.game); Path
p(trying.path); t_game.move(dir); p.push_back(dir); DBG("-- check alive point
"<<dir<<"\n"); if(!t_game.Alive()) continue; DBG("-- check reachable point
"<<dir<<"\n"); if
(!reachable(t_game.Body(),t_game.Body().front(),t_game.Body().back(),t_game.Size()))
        continue;
      DBG("-- check not-closed in point "<<dir<<"\n");
      if(t_game.Body()[0]==food)
        if(wall_count(t_game.Body(),food,t_game.Size())>2 and
distance(food,t_game.Body().back(),1)<3)
          // If eating the food would (probably) cut us off, don't.
          continue;
        else
          return p;
